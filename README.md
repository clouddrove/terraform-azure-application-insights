...
| daily\_data\_cap\_in\_gb | Specifies the Application Insights component daily data volume cap in GB. | `number` | `null` | no |
...
| geo\_locations | Specifies a list of where to physically run the tests from to give global coverage for accessibility of your application. | `list(string)` | `[]` | no |
...
| internet\_query\_enable | (Optional) Should the Application Insights component support querying over the Public Internet? Defaults to true. | `bool` | `true` | no |
| kind | n/a | `string` | `""` | no |
| label\_order | Label order, e.g. sequence of application name and environment `name`,`environment`,'attribute' [`webserver`,`qa`,`devops`,`public`,] . | `list(any)` | `[]` | no |
| list\_of\_test\_locations | List of Azure locations that will perform the specified web tests. Default is set to 5 US locations.  Microsoft recommendation is a minimum of 5 test locations with an alert threshold of N-2. Ref: https://docs.microsoft.com/en-us/azure/azure-monitor/app/monitor-web-app-availability | `list(string)` | <pre>[<br>  "us-ca-sjc-azr",<br>  "us-tx-sn1-azr",<br>  "us-il-ch1-azr",<br>  "us-va-ash-azr",<br>  "us-fl-mia-edge"<br>]</pre> | no |
...
| monitored\_enabled | n/a | `bool` | `false` | no |
